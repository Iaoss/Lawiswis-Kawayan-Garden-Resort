...
// `embedded` is used when this renders inside the Settings page tab,
// so it does not wrap itself in a second PageLayout.
export default function CancellationPolicy({ embedded = false }) {
  ...
  const content = (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2 style={{ fontSize: embedded ? '16px' : '20px', fontWeight: '700', color: TEXT, margin: 0 }}>
          Cancellation processing
        </h2>
        <button onClick={() => setShowPolicyEdit(!showPolicyEdit)}
          style={{ background: ACCENT, color: ACCENT_TEXT, border: 'none', borderRadius: '10px', padding: '9px 18px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', fontFamily: "'Poppins', sans-serif", display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {success && (
        <div style={{ background: SUCCESS_BG, color: SUCCESS_TEXT, padding: '12px 16px', borderRadius: '10px', marginBottom: '16px', fontSize: '13px', fontWeight: '500', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: embedded ? '1fr 320px' : '1fr 360px', gap: '20px' }}>
        {/* Reservation list */}
        <div style={S.card}>
        ...
        <div>
          {!selected ? (
            <div style={{ ...S.card, padding: '40px', textAlign: 'center' }}>
              <i className="ti ti-ban" style={{ fontSize: '32px', color: MUTED, display: 'block', marginBottom: '14px' }} />
              <div style={{ fontWeight: '600', color: TEXT, marginBottom: '6px' }}>No reservation selected</div>
              <div style={{ fontSize: '12px', color: MUTED }}>Pick "Cancel" on a reservation to see the fee breakdown</div>
            </div>
          ) : cancelResult && (
  ...
  if (embedded) {
    return <div style={{ fontFamily: "'Poppins', sans-serif" }}>{content}</div>;
  }
